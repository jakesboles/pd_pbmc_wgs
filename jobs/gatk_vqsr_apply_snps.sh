...
#SBATCH --job-name vqsr_snp_apply
#SBATCH --nodes 1
...
# ---- Step 3: ApplyVQSR (SNP) ----
echo "Applying SNP VQSR model"
${JAVA17_BIN} -Xmx16g -jar ${GATK_DIR}/gatk-package-4.4.0.0-local.jar ApplyVQSR \
   -R ${RESOURCE_DIR}/Homo_sapiens_assembly38.fasta \
   -V gathered_genotyped_gvcf/genotyped_cohort.raw.vcf.gz \
   --recal-file ${OUT_DIR}/cohort.snps.recal \
   --tranches-file ${OUT_DIR}/cohort.snps.tranches \
   --truth-sensitivity-filter-level 99.5 \
   -mode SNP \
   -O ${OUT_DIR}/cohort.snps_recalibrated.vcf.gz